...
class HillClimbing:
    ...
    def executar_estocastico(self, solucao_inicial, max_iteracoes=1000, verbose=False):
        """
        Adaptação da função 'executar' para rodar o método estocástico explicado na atividade 3
        """
        solucao_atual = copy.deepcopy(solucao_inicial)
        fitness_atual = self.funcao_fitness(solucao_atual)
        ...
        while iteracao < max_iteracoes:
            ...
            if vizinhos_melhores:
                solucao_atual = random.choice(vizinhos_melhores)
                fitness_atual = self.funcao_fitness(solucao_atual)
                melhorias += 1
...
if __name__ == "__main__":
    ...
    for i in range(30):
        ...
        melhor_solucao, melhor_fitness, historico = hill_climbing.executar_estocastico(
            solucao_inicial, max_iteracoes=MAX_ITERACOES, verbose=True
        )
        
        lista_historico.append(historico)
        lista_melhor_fitness.append(int(melhor_fitness))

    media_fitness = np.mean(lista_melhor_fitness)
    desvio_padrao_fitness = np.std(lista_melhor_fitness)
    
    print("\n=== RESULTADOS FINAIS ===")
    # print(f"Solução inicial: {solucao_inicial}")
    # print(f"Melhor solução: {melhor_solucao}")
    # print(f"Melhor valor total: {melhor_fitness}")
    print(f"Média de fitness: {media_fitness}")
    print(f"Desvio Padrão de fitness: {desvio_padrao_fitness}")
    ...
    # results = np.savetxt("resultados.txt", lista_historico)